/*
 * KVM Bridge T-Dongle S3 - v5.23
 *
 * NOVITÀ: nessuna modifica allo sketch rispetto alla v5.22.
 * Le impostazioni del mouse (sensibilità, soglia, intervallo)
 * sono gestite interamente dalla PWA (HTML) e non richiedono
 * modifiche al firmware.
 *
 * IMPOSTAZIONI IDE:
 *   Board:       ESP32S3 Dev Module (o LilyGo T-Dongle S3)
 *   Partition:   Huge APP (3MB/1MB LittleFS) ← OBBLIGATORIO
...
// ── Fix C: salva su file con verifica ─────────────────────────
bool salvaSuFile(const char* path, String jsonContent) {
  File f = LittleFS.open(path, "w");
  if (!f) {
    Serial.println("❌ ERRORE: impossibile aprire " + String(path) + " in scrittura");
    return false;
  }
  f.print(jsonContent);
  f.close();
  ...
  if (!check) {
    Serial.println("❌ ERRORE: file non trovato dopo salvataggio: " + String(path));
    return false;
  }
  ...
  if (content != jsonContent) {
    Serial.println("❌ ERRORE: contenuto diverso dopo salvataggio!");
    Serial.println("   Atteso: " + jsonContent);
    Serial.println("   Trovato: " + content);
    return false;
  }

  Serial.println("✅ File salvato e verificato: " + String(path));
  return true;
}
...
void salvaMacroCustom(String jsonItem) {
  Serial.println("📥 salvaMacroCustom() ricevuto: " + jsonItem);
  ...
  if (err) {
    Serial.println("❌ ERRORE JSON macro: " + String(err.c_str()));
    inviaBLE("ERR:JSON_MACRO\n");
    return;
  }
  ...
  if (salvaSuFile(FILE_MACRO, out)) {
    inviaBLE("MACRO_SAVED:" + String(nome) + "\n");
    Serial.println("✅ Macro salvata: " + String(nome));
  } else {
    inviaBLE("ERR:SAVE_MACRO\n");
  }
}
...
void salvaCredenziale(String jsonItem) {
  Serial.println("📥 salvaCredenziale() ricevuto: " + jsonItem);
  ...
  DeserializationError err = deserializeJson(docItem, jsonItem);
  if (err) {
    Serial.println("❌ ERRORE JSON cred: " + String(err.c_str()));
    inviaBLE("ERR:JSON_CRED\n");
    return;
  }
  ...
  String out; serializeJson(docList, out);
  if (salvaSuFile(FILE_CRED, out)) {
    inviaBLE("CRED_SAVED:" + String(nome) + "\n");
    Serial.println("✅ Credenziale salvata: " + String(nome));
  } else {
    inviaBLE("ERR:SAVE_CRED\n");
  }
}
...
void eseguiCredenziale(String nome) {
  Serial.println("🔑 eseguiCredenziale() per: " + nome);
  ...
  Serial.println("❌ Credenziale non trovata: " + nome);
  inviaBLE("ERR:CRED_NOT_FOUND\n");
}

// ── Invia JSON via BLE in chunks ──────────────────────────────
void inviaFileBLE(String prefix, const char* path) {
  File f = LittleFS.open(path, "r");
  if (!f) {
    inviaBLE("ERR:FILE\n");
    Serial.println("❌ FILE non trovato: " + String(path));
    return;
  }
  String content = f.readString();
  f.close();
  Serial.println("📤 " + String(prefix) + " content: " + content);
  ...
  Serial.println("✅ " + String(prefix) + " inviato (" + String(total) + " byte)");
}
...
class ServerCB : public BLEServerCallbacks {
  void onConnect(BLEServer* s) {
    ...
    Serial.println("✅ BLE Connesso — sync in 3s");
  }
  ...
  void onWrite(BLECharacteristic *pC) {
    ...
    for (size_t i = 0; i < len; i++) {
      ...
      if (c == '\n' || c == '\r') {
        ...
        bleBuffer += c;
        if (bleBuffer.length() > 1024) {
          Serial.println("⚠️ Buffer overflow — reset");
          bleBuffer = "";
        }
      }
    }
...
void setup() {
  ...
  delay(500);
  Serial.println("========================================");
  Serial.println("KVM Bridge T-Dongle S3 v5.23");
...
void loop() {
  ...
  if (pendingSync && deviceConnected && (millis() - connectTime > 3000)) {
    pendingSync = false;
    Serial.println("📤 Sync automatico: invio macro e credenziali");
